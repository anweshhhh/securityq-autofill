...
import { getApprovalHistoryForItem } from "@/server/questionHistory/getApprovalHistoryForItem";
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const history = await getApprovalHistoryForItem(ctx, questionnaireId, itemId);
    if (!history) {
      return jsonError({
        status: 404,
        code: "NOT_FOUND",
    ...
    return toApiErrorResponse(error, "Failed to load approval history.");
  }
}
