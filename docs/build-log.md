...
## 2026-03-03 - ui research + D-Dark shell polish pass

- Added UI design spec:
  - `docs/ui-spec.md`
  - scope includes IA, shell/workbench layout rules, component rules, interaction states, a11y targets, and top-10 issue->fix mapping.
  - research basis captured in spec:
    - Atlassian navigation best practices
    - shadcn dashboard/sidebar patterns
    - WCAG contrast minimum
    - Material drawer behavior principles (using provided distilled guidance where docs were JS-gated)
- Navigation cleanup (`ui-spec` implementation):
  - reduced primary sidebar nav to core task routes (`Documents`, `Questionnaires`, `Members`)
  - moved secondary destinations to top-nav overflow (`More`):
    - `Home`
    - `Ask` (DEV only)
  - strengthened active sidebar affordance (contrast + vertical indicator) and collapse behavior polish.
- Workbench hierarchy/actions consistency (`/questionnaires/[id]`):
  - improved question rail summary (`visible/total` + status metric badges)
  - normalized trust-bar action hierarchy:
    - `Run Autofill` as primary
    - bulk/reuse approvals as secondary
    - export/help as tertiary
  - standardized answer-panel and quick-action toolbars with consistent spacing/wrapping.
- Evidence panel spacing + primitive standardization:
  - tightened evidence toolbar/icon action density
  - normalized chip/action spacing and control sizes
  - standardized token-driven spacing/radius/shadow/line styles through `src/app/ui-refine.css`
  - bounded answer/snippet containers now preserve line breaks and avoid layout instability for long text.
- Accessibility updates:
  - shell landmarks reinforced (`aside` + labeled nav + single `main`)
  - in-button autofill progress now includes explicit `progressbar` semantics.
- UI audit tooling updates:
  - updated `scripts/ui_audit_questionnaire.js` assertions with shell checks:
    - `primary_nav`
    - `main_landmark`
  - workbench assertions remain enabled for questionnaire-details URLs.
- Validation:
  - `npm run ui:audit -- http://localhost:3000/login`
    - artifacts: `artifacts/ui-audit/2026-03-03T07-56-11-698Z`
    - console errors/warnings: `0`
    - network failures: `0`
    - axe violations: `0` (critical/serious: `0`)
  - `npm test` => PASS
  - `npm run build` => PASS
